...
### FIXME: currently a lot of more or less quivalent code is used within the
### methods based on the stabletree function itself;
### write external functions and call them within each method?
### also, most functions in stabletree are not pure functions
...
as.stabletree.randomForest <- function(x, applyfun = NULL, cores = NULL, ...) {
  ...
  ##FIXME: process sampler
  sampler <- list(method = "randomForest::randomForest",
    sampler = "randomForest::randomForest")
  B <- x$forest$ntree
  ...
  extract_split <- function(x) {
    ...
    ## the subsample and there is no easy fix
  ...
  ##FIXME: save trees? Actually not really usable, so we set trees to NULL
  rval <- list(
    call = call,
    B = B,
    sampler = sampler,
...
as.stabletree.RandomForest <- function(x, applyfun = NULL, cores = NULL, ...) {
  ...
  extract_breaks <- function(x) {
    sp <- extract_split(x)
    if(!is.null(sp)) {
      vi <- sapply(sp, "[[", "varid") ## ! no -1L see extract_split
    } else vi <- NULL
    br <- lapply(sp, "[[", "breaks")
    id <- lapply(sp, "[[", "index")
    names(id) <- names(br) <- x_names[vi]
  ...
  ## build stabletree object
  ##FIXME: save trees? Actually not really usable, so set trees to NULL
  rval <- list(
    call = call,
    B = B,
    sampler = sampler,
...
as.stabletree.cforest <- function(x, applyfun = NULL, cores = NULL,
  savetrees = FALSE, ...) {
  call <- try(getCall(x), silent = TRUE)
  ##FIXME: process sampler
  sampler <- list(method = "partykit::cforest", sampler = "partykit::cforest")
